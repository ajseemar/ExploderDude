class CollisionDetector {
    ...
    static resolveCollision(player, entity) {
        ...
        if (absDX > absDY) {
            if (dx < 0) {
                player.position.x = entity.getRight(); // - (player.bbox.tl - player.position.x); // - player.bbox.tl(player.size / 4);

            } else {
                player.position.x = entity.getLeft() - player.width; // + (player.bbox.tl - player.position.x); // + player.size / 4; // + player.size / 4;
                // player.position.x = entity.getLeft() + player.size / 4;
            }
        } else {
            if (dy < 0) {
                player.position.y = entity.getBottom(); // - (player.bbox.tr - player.position.y); // - player.size / 6;
            } else {
                player.position.y = entity.getTop() - player.height; // + (player.bbox.tr - player.position.y); // + player.size / 6;
            }
        }
    }
}
